...
set -euo pipefail
cd "$(cd "$(dirname "$0")/.." && pwd)"
export FOUNDRY_MEMORY_LOCAL_ROOT="$(mktemp -d)"
...
uv run foundry-memory search "tests fail ModuleNotFoundError settlement_pb2" \
    --backend local --project contoso-payments --top 1

banner "3. The hypothesis, measured (full report: benchmark/RESULTS.md)"
uv run foundry-memory bench | sed -n '/| question/,/| \*\*total\*\*/p'

banner "4. Dream: curate a messy store (2 stale duplicates) + mine 2 transcripts"
uv run foundry-memory seed samples/memories-messy --backend local --project messy > /dev/null
...
uv run foundry-memory search "429 parallel tests" --backend local --project curated --top 1
