...
set -euo pipefail
cd "$(cd "$(dirname "$0")/.." && pwd)"
export TEAMLORE_LOCAL_ROOT="$(mktemp -d)"
...
uv run lore search "429 parallel tests" --backend local --project curated --top 1
